...
يستخدم Memesplora ent ORM لإدارة قاعدة البيانات، ويدعم SQLite (للتطوير / الجهاز الواحد) و PostgreSQL (للإنتاج). تخزن قاعدة البيانات البيانات الوصفية فقط، بينما يتم تخزين محتوى الملفات الفعلي في نظام الملفات في الذاكرة.
...
func (User) Fields() []ent.Field {
    return []ent.Field{
        ...
        field.Int64("max_storage").Default(0), // 0 = غير محدود
...
| البيئة | قاعدة البيانات | الوصف |
|:----:|:------:|------|
| التطوير | SQLite | بدون إعدادات إضافية، قاعدة بيانات ملفية |
| النشر على جهاز واحد | SQLite | مناسبة لاستخدام مستخدم واحد |
| النشر في الإنتاج | PostgreSQL | تدعم التزامن، مناسبة لعدة مستخدمين |
