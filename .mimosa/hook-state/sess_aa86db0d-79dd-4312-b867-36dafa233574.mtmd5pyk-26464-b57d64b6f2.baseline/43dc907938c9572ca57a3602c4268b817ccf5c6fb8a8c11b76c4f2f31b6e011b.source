...
func NewRAMDevice(id string, size uint64) (*RAMDevice, error) {
	...
	var data []byte
	if size > 0 {
		// addr is a valid OS pointer from VirtualAlloc, conversion is safe
		sh := (*[1 << 30]byte)(unsafe.Pointer(addr))[:size:size]
		data = sh
	}
...
func (d *RAMDevice) Close() error {
	d.mu.Lock()
	defer d.mu.Unlock()
	// Get the underlying pointer from the slice
	if len(d.data) > 0 {
		return windows.VirtualFree(uintptr(unsafe.Pointer(&d.data[0])), 0, windows.MEM_RELEASE)
	}
	return nil
}